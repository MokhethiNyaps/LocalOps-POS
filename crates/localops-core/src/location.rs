...
pub fn get_location(connection: &Connection, id: &str) -> Result<Option<Location>> {
    ...
    let location = stmt.query_row([&id], |row| {
        Ok(Location {
            id: row.get(0)?,
...
pub fn get_location_by_code(connection: &Connection, business_id: &str, code: &str) -> Result<Option<Location>> {
    let mut stmt = connection.prepare(
        "SELECT id, business_id, name, code, address, contact_phone, contact_email, active 
         FROM locations WHERE business_id = ?1 AND code = ?2 AND active = 1"
    )?;
    let location = stmt.query_row([&business_id, &code], |row| {
        Ok(Location {
            id: row.get(0)?,
            business_id: row.get(1)?,
            name: row.get(2)?,
...
pub fn deactivate_location(connection: &Connection, id: &str) -> Result<()> {
    connection.execute(
        "UPDATE locations SET active = 0 WHERE id = ?1",
        [id],
    )?;
    Ok(())
}
...
pub fn list_locations(connection: &Connection, business_id: &str) -> Result<Vec<Location>> {
    ...
    locations.collect::<std::result::Result<Vec<_>, _>>().map_err(Into::into)
}
...
mod tests {
    ...
    fn updates_location_details() {
        ...
        assert_eq!(location.contact_email, Some("store@example.com".to_string()));
    }
    ...
    fn gets_location_by_code() {
        ...
        let location = get_location_by_code(&db, &business_id, "MAIN").unwrap().unwrap();
        assert_eq!(location.name, "Main Store");
        assert_eq!(location.code, Some("MAIN".to_string()));
