...
pub fn create_terminal(
    ...
    if let Some(loc_id) = location_id {
        let loc_exists: bool = connection.query_row(
...
pub fn get_terminal(connection: &Connection, id: &str) -> Result<Option<Terminal>> {
    ...
    let terminal = stmt.query_row([&id], |row| {
        Ok(Terminal {
            id: row.get(0)?,
...
pub fn get_terminal_by_code(connection: &Connection, business_id: &str, code: &str) -> Result<Option<Terminal>> {
    let mut stmt = connection.prepare(
        "SELECT id, business_id, location_id, name, code, description, active 
         FROM terminals WHERE business_id = ?1 AND code = ?2 AND active = 1"
    )?;
    let terminal = stmt.query_row([&business_id, &code], |row| {
        Ok(Terminal {
            id: row.get(0)?,
            business_id: row.get(1)?,
            location_id: row.get(2)?,
...
pub fn deactivate_terminal(connection: &Connection, id: &str) -> Result<()> {
    connection.execute(
        "UPDATE terminals SET active = 0 WHERE id = ?1",
        [id],
    )?;
    Ok(())
}
...
pub fn list_terminals(connection: &Connection, business_id: &str) -> Result<Vec<Terminal>> {
    ...
    terminals.collect::<std::result::Result<Vec<_>, _>>().map_err(Into::into)
}

/// List terminals for a specific location
pub fn list_terminals_by_location(connection: &Connection, location_id: &str) -> Result<Vec<Terminal>> {
    let mut stmt = connection.prepare(
        "SELECT id, business_id, location_id, name, code, description, active 
         FROM terminals 
...
mod tests {
    use super::*;
    use crate::{open_memory_database, business, location};
    ...
    fn creates_terminal_with_location() {
        let db = open_memory_database().unwrap();
        let business_id = business::create_business(&db, "Test Business").unwrap();
        let location_id = location::create_location(&db, &business_id, "Main Store", Some("MAIN")).unwrap();
        
        let id = create_terminal(&db, &business_id, Some(&location_id), "Store Register", Some("STORE01")).unwrap();
    ...
    fn updates_terminal_details() {
        ...
        assert_eq!(terminal.location_id, Some(location_id));
        assert_eq!(terminal.description, Some("Updated description".to_string()));
    }
    ...
    fn gets_terminal_by_code() {
        ...
        let terminal = get_terminal_by_code(&db, &business_id, "MAIN").unwrap().unwrap();
        assert_eq!(terminal.name, "Main Register");
    ...
    fn lists_terminals_by_location() {
        let db = open_memory_database().unwrap();
        let business_id = business::create_business(&db, "Test Business").unwrap();
        let location_a = location::create_location(&db, &business_id, "Location A", Some("LOCA")).unwrap();
        let location_b = location::create_location(&db, &business_id, "Location B", Some("LOCB")).unwrap();
        ...
        assert!(terminals_a.iter().all(|t| t.location_id == Some(location_a.clone())));
        assert!(terminals_b.iter().all(|t| t.location_id == Some(location_b.clone())));
    }
